...
class FangXfSpider(CrawlSpider):
    ...
    start_urls = [
                  'https://gz.newhouse.fang.com/house/s/a77/',  #广州 586条
    ...
    rules = (
        Rule(LinkExtractor(allow=r'.*/house/s/a', restrict_xpaths="//li[@class='fr']"), follow=True),
    ...
    def parse_detail(self, response):
        ...
        # #### 基本信息
        # item['价格'] = response.xpath("//div[text()='价')]/following-sibling::div/em/text()").get().strip()
        ...
        # item['开发商']   = response.xpath("//div[text()='开' and @class='list-left']/following-sibling::div/a/text()").get().strip()
        ...
        # item['楼盘优惠'] = response.xpath("//div[contains(text(),'楼盘优惠')]/following-sibling::div/text()").get().strip()
        ...
        # #### 小区规划
        # item['占地面积']    = response.xpath("//div[contains(text(),'占地面积')]/following-sibling::div//text()").get().strip()
        # item['建筑面积']    = response.xpath("//div[contains(text(),'建筑面积')]/following-sibling::div//text()").get().strip()
        # item['容积率']      = response.xpath("//div[text()='容' and @class='list-left']/following-sibling::div/text()").get().strip()
        # item['绿化率']      = response.xpath("//div[text()='绿' and @class='list-left']/following-sibling::div/text()").get().strip()
        # item['停车位']      = response.xpath("//div[contains(text(),'停') and @class='list-left']/following-sibling::div/text()").get().strip()
        # item['物业公司']    = response.xpath("//div[contains(text(),'物业公司')]/following-sibling::div/a/text()").get().strip()
        # item['楼层状况']    = response.xpath("//div[contains(text(),'楼层说明') or contains(text(),'楼层状况')]/following-sibling::div//text()").get().strip()
        # item['物业费']      = response.xpath("//div[(text()='物' or text()='物业费：') and @class='list-left']/following-sibling::div/text()").get().strip()
